...
const name = ref('');
const guard_name = ref('');

function submit() {
  router.post('/administracion/permision', {
    name: name.value,
...
<template>
  ...
  <AppLayout>
    <div class="w-full px-8 py-10">
      <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-10">Crear Permiso</h1>
      ...
      <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <!-- Email -->
        <div class="col-span-1">
          <label for="guard_name" class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Guard Name</label>
          <input
            id="guard_name"
            v-model="guard_name"
            type="text"
